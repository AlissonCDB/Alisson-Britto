...
const OpcoesPaginasContainer = styled.ul`
    ...
    &.fixed{
        ...
        align-items: center;
        z-index: 99;
        /* Atualizado com o $ para referenciar as transient props */
...
export const OpcoesPaginas = ({ valor }) => {
    ...
    return (
        <OpcoesPaginasContainer
            ...
            /* Utilizando ternário para evitar enviar 'false' para a DOM */
            $borda={!borderPosition ? 'none' : undefined}
            $sombra={!borderPosition ? 'none' : undefined}
        >
            {opcoes.map((opcao, index) => (
                /* Adicionada a key obrigatória do React */
                <Opcoes key={index}>
                    {/* Verificação para apontar 'HOME' para a raiz '/' e não '/home' */}
                    <Link href={opcao === 'HOME' ? '/' : `/${opcao.toLowerCase()}`}>
                        <p>{opcao}</p>
                    </Link>
                </Opcoes>
            ))}
